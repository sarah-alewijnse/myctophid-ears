...
mod_list <- list(
  T_obs = PRM_tidy$mean_Temp,
  T_se = PRM_tidy$se_Temp,
...
model_PRM_crush <- map2stan(
  alist(
    T_est ~ dnorm(mu , sigma),
    
    # Linear model
    mu <- a + b_C*Crushed,
    
    # Data uncertainties
    T_obs ~ dnorm(T_est, T_se),
  ...
  data = mod_list,
  start = list(T_est = mod_list$T_obs),
  WAIC = FALSE,
  iter = 10000,
  warmup = 5000,
...
mod_list <- list(
  T_obs = myct_tidy$mean_Temp,
  T_se = myct_tidy$se_Temp,
