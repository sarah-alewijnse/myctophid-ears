...
avg <- mean(myct$mean_M, na.rm = TRUE)
sd <- sd(myct$mean_M, na.rm = TRUE)

ks.test(myct$mean_M, "pnorm", avg, sd)

# Normal distribution ok

## Check for homogeneity of variances with Levene's test

leveneTest(mean_M ~ sciname, data = myct)

# Homogeneity of variances not ok

## Proceed with Kruska-Wallis Test

kruskal.test(mean_M ~ sciname, data = myct)
...
myct_na <- dplyr::select(myct, sciname, mean_M)
myct_na <- na.omit(myct_na)

mean_M <- myct_na$mean_M
sciname <- myct_na$sciname

dunnTest(mean_M ~ sciname,
         method = "bonferroni")
